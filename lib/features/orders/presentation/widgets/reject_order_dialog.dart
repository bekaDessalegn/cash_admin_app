...
Widget rejectOrderDialog({required BuildContext context, required String orderId}){
  return Dialog(
    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0)),
    child: BlocConsumer<PatchOrderBloc, PatchOrderState>(builder: (_, state){
      if(state is RejectOrderStateLoading){
  ...
  return SizedBox(
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          Text("Reject order", style: TextStyle(
              color: onBackgroundColor,
              fontSize: 22,
              fontWeight: FontWeight.bold
          ),),
          SizedBox(height: 10,),
          Text("Are you sure you want to reject this order ?", style: TextStyle(
              color: onBackgroundColor,
              fontSize: 16
          ),),
          SizedBox(height: 20,),
          Row(
            ...
            children: [
              ...
              TextButton(onPressed: (){
                final reject = BlocProvider.of<PatchOrderBloc>(context);
                reject.add(RejectOrderEvent(orderId));
              }, child: isLoading ? SizedBox(
                  width: 16,
                  height: 16,
                  child: CircularProgressIndicator(color: primaryColor,)) : Text("Reject", style: TextStyle(
                  color: primaryColor,
                  fontSize: 16
              ),),),
            ],
